Feature: Login User
  User visits the Login Page, files in the form, and submits
  ...
  Tests that the 'Login' button is disabled when either input elements contain invalid values
    ...
    Examples:
      ...
      | Invalid Email  | invalid-email | abcd1234qwery |
      | Short Password | valid@ema.il  | shortpw       |

  Scenario: Valid Input
  Tests that the 'Login' button is enabled when valid values are provided,
  and that upon successful login, the UI display will display the message "You've been logged in successfully"
    ...
    Then the '#login-button' element should not have a 'disbled' attribute
#
